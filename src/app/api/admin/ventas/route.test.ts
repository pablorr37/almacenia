...
describe("GET /api/admin/ventas", () => {
  ...
  beforeEach(async () => {
    admin = await registrarUsuario({
      email: `test-admin-ventas-route-${Date.now()}@almacenia.test`,
    ...
    vendedor = await registrarUsuario({
      email: `test-admin-ventas-route-v-${Date.now()}@almacenia.test`,
      password: "password123",
      nombre: "Vendedor",
    });
  ...
  it("403 si no es admin", async () => {
    ...
    const res = await GET(new NextRequest("http://localhost/api/admin/ventas"));
    expect(res.status).toBe(403);
  });

  it("200 lista ventas con tiendaNombre y total", async () => {
    obtenerUsuarioActualMock.mockResolvedValue(admin);
    const res = await GET(new NextRequest(`http://localhost/api/admin/ventas?tiendaId=${tienda.id}`));
