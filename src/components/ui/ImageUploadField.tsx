...
export function ImageUploadField({ tipo, entidadId, valorActual, onSubido, label }: ImageUploadFieldProps) {
  ...
  async function manejarCambio(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // LIMITE_FOTOS_PLAN_FREE (10-planes.md) ya trae el mensaje de upsell armado
      // desde el backend; el resto de los errores usa un mensaje genérico.
      setError(
        err instanceof ApiError && err.code === "LIMITE_FOTOS_PLAN_FREE"
          ? err.message
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-3">
        ...
        <label className="cursor-pointer rounded-control border border-border bg-surface px-3.5 py-2.5 text-[13px] font-semibold text-text">
          {subiendo ? "Subiendo..." : valorActual ? "Cambiar foto" : "Subir foto"}
          <input
            type="file"
